package com.timsmith.responder.GeoUtils;
...
public class GeoUtils {
    ...
    public static double distance(double lat1, double long1, double lat2, double long2) {
        ...
        final double radius = (com.timsmith.responder.GeoUtils.Constants.EARTH_EQ_RADIUS + com.timsmith.responder.GeoUtils.Constants.EARTH_POLAR_RADIUS)/2;
    ...
    public static double distanceToLatitudeDegrees(double distance) {
        return distance/ com.timsmith.responder.GeoUtils.Constants.METERS_PER_DEGREE_LATITUDE;
    }

    public static double distanceToLongitudeDegrees(double distance, double latitude) {
        double radians = Math.toRadians(latitude);
        double numerator = Math.cos(radians) * com.timsmith.responder.GeoUtils.Constants.EARTH_EQ_RADIUS * Math.PI / 180;
        double denominator = 1/Math.sqrt(1 - com.timsmith.responder.GeoUtils.Constants.EARTH_E2*Math.sin(radians)*Math.sin(radians));
        double deltaDegrees = numerator*denominator;
        if (deltaDegrees < com.timsmith.responder.GeoUtils.Constants.EPSILON) {
